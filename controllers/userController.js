...
const UserController = {
  ...
  updateAssignRole: async (req, res) => {
    ...
    try {
      ...
      const [roleExists] = await db.query("SELECT id FROM roles WHERE id = ?", [
        roleId,
      ]);
    ...
    try {
      ...
      if (Array.isArray(rolePermissions) && rolePermissions.length > 0) {
        ...
        const uniquePermissions = rolePermissions.reduce((acc, item) => {
          if (
            item.permission_id &&
            !acc.some((perm) => perm.id === item.permission_id)
          ) {
            acc.push({
  ...
  updateUserDetail: async (req, res) => {
    ...
    const { name, email, roles, role_id, permissions, password } = req.body;
    ...
    let roleIds = parseRoles(roles);

    // Tambahan fallback jika hanya ada role_id dari frontend
    if ((!roleIds || roleIds.length === 0) && role_id) {
    ...
    try {
      ...
      if (roleIds.length > 0) {
        ...
        if (permissionIds.length > 0) {
          ...
          const permissionUpdateResult = await User.updatePermissions(
            [roleIds[0]],
            permissionIds
          );
      ...
      userData.forEach((row) => {
        ...
        if (row.permission_id) {
          const existingPermission = permissionsArray.find(
