...
const Patient = {
  ...
  create: async (data) => {
    ...
    await db.query(
      "UPDATE patients SET patient_code = ?, barcode = ? WHERE id = ?",
  ...
  getAllWithPagination: async (limit, offset, search = "") => {
    const query = `
        ...
        LEFT JOIN mapping_patients mp 
            ...
            AND mp.is_order = 0
        WHERE 
            (p.name LIKE ? OR p.patient_code LIKE ? OR p.barcode LIKE ? OR p.nik LIKE ? OR p.no_rm LIKE ?)
            AND p.id != 0
    ...
    const result = rows.map((row) => {
      ...
      patient.lab_number = lab_number_string
        ? lab_number_string.split(",")
        : [];
      return patient;
    });
  ...
  getTotalCount: async () => {
    const [rows] = await db.query(
      `SELECT COUNT(*) AS count FROM patients WHERE id != 0`
    );
    return rows[0].count;
  },
  ...
  getByMonth: async (month) => {
    const [rows] = await db.query(
      `SELECT * FROM patients WHERE MONTH(created_at) = ?`,
      [month]
    );
    return rows;
  },
